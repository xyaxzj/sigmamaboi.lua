-- ==============================================================================
-- 🌦️ KALB WEATHER RADAR & PREDICTOR PRO (100% ACCURATE & INTEGRATED V4)
-- ==============================================================================
-- 📋 Data Resmi Game Terintegrasi:
-- 1. ⚡ Live Active Weather:
--    - Membaca langsung dari ReplicatedStorage.Modules.ServicesLoader.WeatherService_Client.Events
--    - Terhubung ke rev_WeatherUpdate & rev_AddedWeather
-- 2. 🔮 Next Event Countdown:
--    - Membaca waktu hitung mundur dari Workspace["Admin Machine"] ("NEXT EVENT: 10:27")
--    - Menghitung waktu rotasi cuaca berikutnya dari waktu selesai event aktif
-- 3. 🎰 Weather Machine Stock Monitor:
--    - Membaca ReplicatedStorage.Modules.ServicesLoader.WeatherMachineService.Stock
-- 4. 📚 Database Lengkap 33 Event Cuaca (ReplicatedStorage.Shared.Data.WeatherData):
--    - Back to School, Special Events, Lucky Blocks & Speed Upgrades
-- 5. 📱 Compact UI (280x300px) & Floating Pill Minimize:
--    - Ramping, tidak memakan layar, touch & drag responsive
-- ==============================================================================
...
-- ==============================================================================
-- 📚 DATABASE 33 EVENT RESMI GAME (SESUAI WEATHERDATA)
-- ==============================================================================
local WEATHER_METADATA = {
    ...
    ["Block Cup"] = {
        Title = "Block Cup Event",
        Category = "🏆 Tournament",
        Color = Color3.fromRGB(255, 190, 20),
        Tag = "🏆 Block Cup",
        Image = "rbxassetid://118706576986202",
        Desc = "Kejuaraan turnamen Block Cup di server!"
    },
    ["Block Cup Party"] = {
        ...
        Tag = "🏆 Cup Party",
        Image = "rbxassetid://118706576986202",
        Desc = "Pesta perayaan kejuaraan Block Cup!"
    },
    ["Wandering Trader"] = {
...
-- ==============================================================================
-- 🧠 2. STATE DETECTOR & REAL-TIME ENGINE
-- ==============================================================================
...
-- Pembaca Data Cuaca Terlengkap (Events, Admin Machine & Machine Stock)
local function getUnifiedWeatherData()
    ...
    -- 1. Baca WeatherService_Client.Events
    ...
    -- Fallback via getgc() jika Events kosong tapi ada di memory
    ...
    -- 2. Baca WeatherMachineService.Stock
...
-- Refresh Tampilan (Pembersihan Bersih & Akurat)
local function refreshView()
    ...
    if activeTab == "Live" then
        if #liveList == 0 then
            local bannerSub = adminNextEventStr and string.format("Info Mesin Server: %s", adminNextEventStr) or "Menunggu event cuaca baru dimulai oleh server."
    ...
    elseif activeTab == "Upcoming" then
        ...
        -- 1. Timer Pasti dari Admin Machine Server
        if adminNextEventStr then
            renderBannerCard("⏳ " .. adminNextEventStr, "Hitung mundur resmi event berikutnya dari Admin Machine server.", orderIndex)
            orderIndex = orderIndex + 1
        elseif #liveList > 0 then
            local first = liveList[1]
            local bannerTitle = string.format("🎲 ROLL CUACA BERIKUTNYA: %s", formatTime(first.Remaining))
            local bannerSub = string.format("Event baru akan di-roll server pukul %s WIB (setelah '%s' selesai).", os.date("%H:%M:%S", first.EndTime), first.Name)
            renderBannerCard(bannerTitle, bannerSub, orderIndex)
            orderIndex = orderIndex + 1
        else
            renderBannerCard("🎲 STATUS CUACA: STANDBY", "Server sedang dalam interval jeda cuaca...", orderIndex)
            orderIndex = orderIndex + 1
        end

        -- 2. Stok Weather Machine
        ...
        -- 3. Daftar Pool 33 Event Resmi Game
        renderHeaderSection("🌟 POOL ROTASI EVENT RESMI:", orderIndex)
        orderIndex = orderIndex + 1

        local rotationOrder = {
            "MathEvent", "PEClass", "LiftMachine", "StealHomework",
        ...
        for _, pName in ipairs(rotationOrder) do
...
print("--------------------------------------------------")
print("✅ [KALB] Weather Radar Pro V4 (100% Accurate) Siap Digunakan!")
