-- ==========================================================
-- MOCTA TRADE AUTOMATOR V15.7 (SYNTAX & PARSER FIXED)
-- ==========================================================
...
local function getFullItemName(tool)
    ...
    if mutValue then 
        displayName = displayName .. " [" .. tostring(mutValue) .. "]" 
    end  
    ...
    if lvlValue then 
        displayName = displayName .. " (Lv." .. tostring(lvlValue) .. ")" 
    end
    return displayName
end
                    ...
                    if string.find(text, "accept") or string.find(text, "yes") or string.find(text, "trade") then
                        pcall(function()
                            if getconnections then
                                ...
                                if conns1 then 
                                    for _, conn in ipairs(conns1) do pcall(function() conn:Fire() end) end 
                                end
                                
                                local conns2 = getconnections(desc.Activated)
                                if conns2 then 
                                    for _, conn in ipairs(conns2) do pcall(function() conn:Fire() end) end 
                                end
                            end
                            if firesignal then
...
local Window = Rayfield:CreateWindow({
    Name = "Mocta Trade V15.7", 
    LoadingTitle = "Syntax Fixed Edition...", 
...
TabQueue:CreateButton({
    ...
    Callback = function()
        if TargetPlayerName == "" then 
            return Rayfield:Notify({Title = "Error", Content = "Pilih pembeli dulu di atas!", Duration = 2}) 
        end
        ...
        if itemsFound == 0 then 
            Rayfield:Notify({Title = "Kosong", Content = "Tidak ada Bacon Materials.", Duration = 3})
        else 
...
TabQueue:CreateButton({
    ...
    Callback = function() 
        if SelectedMixItem ~= "" and SelectedMixItem ~= "[ANY ASSET]" and SelectedMixQty > 0 then 
            ...
            if cur + SelectedMixQty > rs then 
                ShoppingCart[SelectedMixItem] = rs 
            else 
                ShoppingCart[SelectedMixItem] = cur + SelectedMixQty 
            end 
            updateCartDisplay() 
        end 
    end
...
TabQueue:CreateButton({
    ...
    Callback = function() 
        if SelectedMixItem ~= "" and SelectedMixItem ~= "[ANY ASSET]" then 
            ShoppingCart[SelectedMixItem] = getRealStock(SelectedMixItem) 
            updateCartDisplay() 
        end 
    end
})
...
TabQueue:CreateButton({
    ...
    Callback = function() 
        if TargetPlayerName == "" then 
            return Rayfield:Notify({Title = "Error", Content = "Pilih pembeli dulu!", Duration = 2}) 
        end
        ...
        local needed = {} 
        for k,v in pairs(ShoppingCart) do needed[k] = v end
...
local function updateProgressUI() 
    LiveProgress:Set({Title = "⚡ Auto Sender Progress", Content = string.format("Sisa Item di Antrean: %d\nItem Terkirim: %d", #CurrentQueue, ItemsProcessed)}) 
end

local function setLog(txt) 
    ActionLog:Set({Title = "📜 Live Trade Log", Content = txt}) 
end
...
local function executeSenderBatch()
    ...
    for i = 1, batchSize do 
        table.insert(batch, table.remove(CurrentQueue, 1)) 
    end
    ...
    setLog("Fase 4: Menekan Accept Pertama...")
    ...
    while tradeFrame and tradeFrame.Parent and tradeFrame.Visible do
        ...
        if p1Frame then
            ...
            if p1Confirm and not p1Confirm.Visible then 
                break 
            end 
        end
        task.wait(0.2) 
        ...
        if waitTimeout > 60 then 
            setLog("❌ ERROR: Stuck di Fase 4 kelamaan!") 
            IsProcessing = false 
            return false 
        end
    end
    ...
    if tradeFrame and tradeFrame.Parent and tradeFrame.Visible then
        setLog("Fase 5: P2 Accept. Menunggu Lock 5.5 detik kedua...")
        task.wait(5.5)
        
        setLog("Fase 6: Menekan Final Confirm! Selesai.")
        r_trade_i:FireServer("Confirm") 
        while tradeFrame and tradeFrame.Parent and tradeFrame.Visible do 
            task.wait(0.5) 
        end
...
TabControl:CreateButton({
    Name = "▶️ RUN 1 BATCH SEBAGAI P1", 
    Callback = function() task.spawn(executeSenderBatch) end
})

TabControl:CreateToggle({
    Name = "🔁 FULL AUTO LOOP (P1)", 
    CurrentValue = false, 
            ...
            task.spawn(function() 
                while AutoLoopEnabled do 
                    if #CurrentQueue == 0 then 
                        setLog("🏁 Antrean kosong. Loop berhenti.") 
